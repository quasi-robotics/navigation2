...
PlannerServer::on_configure(const rclcpp_lifecycle::State & state)
{
  ...
  int path_validation_start_offset = node->declare_or_get_parameter("path_validation_start_offset", 0);
  is_path_valid_service_ = std::make_unique<IsPathValidService>(
    shared_from_this(), costmap_ros_, params_->costmap_update_timeout, path_validation_start_offset);
